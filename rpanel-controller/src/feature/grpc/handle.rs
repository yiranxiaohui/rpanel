...
pub async fn handle_upload_status_message(docker_request: DockerRequest) {
    if let Ok(status) = serde_json::from_str::<Status>(docker_request.payload.as_str()) {
        ...
        let system_status = t_system_status::ActiveModel {
            id: Default::default(),
        ...
        t_system_status::Entity::insert(system_status)
            .on_conflict(
                OnConflict::column(t_system_status::Column::AgentId).update_columns([
                    t_system_status::Column::CpuUsage,
                    t_system_status::Column::MemUsed,
                    t_system_status::Column::MemTotal,
                    t_system_status::Column::DiskUsed,
                    t_system_status::Column::DiskTotal,
                    t_system_status::Column::UpdateTime
                ]).to_owned(),
            ).exec(&db)
            .await
